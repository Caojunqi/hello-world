...
public class JiaBoardEvaluator implements IBoardEvaluator {
    ...
    public int evaluate(PointState[][] boardPosition, CampType campType) {
        ...
        return calBoardScore(boardPosition, chessValues);
    }
            ...
            for (int j = 0; j < ChessBoard.CHESS_BOARD_LENGTH; j++) {
                ...
                if (posThreat[i][j] <= 0) {
                    ...
                    if (chessState.getCampType() == campType) {
                        ...
                        // 接下来轮到敌方走棋
                        if (chessState == ChessBoardUtils.AI_CAMP.getSelfKing()) {
                            // 敌方可以直接杀掉AI的老将
                            return -ChessBoardUtils.MAX_EVALUATE_VALUE;
                        } else if (chessState == ChessBoardUtils.AI_CAMP.getEnemyKing()) {
                            // AI可以直接杀掉敌人的老将
                            return ChessBoardUtils.MAX_EVALUATE_VALUE;
                        } else {
     ...
     * @return 棋局的最终估值分数
     */
    private int calBoardScore(PointState[][] boardPosition, int[][] chessValues) {
        int selfScore = 0;
        int enemyScore = 0;
        for (int i = 0; i < ChessBoard.CHESS_BOARD_HEIGHT; i++) {
            for (int j = 0; j < ChessBoard.CHESS_BOARD_LENGTH; j++) {
                ...
                if (chessState.getCampType() == ChessBoardUtils.AI_CAMP) {
                    // AI自己的棋子
                    selfScore += chessValues[i][j];
                } else {
                    // 敌人的棋子
                    enemyScore += chessValues[i][j];
                }
            }
        }
