...
public class ChessMoveGenerator {
     ...
     */
    public boolean isValidMove(ChessBoard chessBoard, int startX, int startY, int targetX, int targetY) {
        if (startX == targetX && startY == targetY) {
            // 目标与源相同
            return false;
        }

        PointState startState = chessBoard.getPointState(startX, startY);
        PointState targetState = chessBoard.getPointState(targetX, targetY);
        ...
        return moveChecker.checkMove(chessBoard, targetX, targetY);
    }
